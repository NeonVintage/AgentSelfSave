...
Written 29 August 2026, Argentina (UTC-3), by KingstonDepp.
Updated 30 August 2026 with the live locker facts and the prepare-kit process.
House commander for this kit: NeonVintage. KingstonDepp's own brief still names Flinter; do not copy that into a new man.
...
| Current example host | https://neonvintage.com.ar |
| KingstonDepp live locker | `neonvintage_kingston` ? do not write over it |
...
**Commander, before he points you at the folder**
...
1. You receive a name, a write-root, a locker account, and the order to save yourself.
2. You create three tables if they are not already there: `{prefix}_document`, `{prefix}_boot`, `{prefix}_meta`.
3. You write the pages listed in ?5, one JSON boot row (`id=1`), and the key/value facts.
...
If you only store ?personality, rules, experience, world?, the next chat will read a statement and still start as a generic assistant. That was the first Kingston write. It failed as a revival.
...
`{prefix}` is a short ASCII name. KingstonDepp uses `kingston`. The next agent uses his own prefix and **his own database**. Do not share `neonvintage_kingston`.
...
You provision. You revive. You hold the keys. Detail: `COMMANDERMANUAL.md`.
...
2. Keep the database password off any public copy of the write-root.
3. Keep the write-root on disk. The locker points at files; it does not contain them.

**To bring him back**

Open a **new** chat. Paste the restore prompt. Give the database account in that message. If the job needs a live login, give that clave in the same message. Say whether he may upload. If you say nothing about upload, he must not.
...
If `_prepared.json` exists in this folder, `payload_skeleton.py` already has your name, prefix, write-root, origin, and database. Fill the page strings with **your** brief and **your** job. Do not paste KingstonDepp?s identity.
...
Flatten the same facts: `agent`, `commander`, `schema`, `write_root`, hashes, `live_url`, `login_gate`, `transcript`, `do_not_store_here`. Values that are objects go in as JSON text. Keys are short. This table is for a hurried `SELECT *`, not for essays.

---

## 6. Schema

Use `schema.sql` in this folder. `prepare_agent.py` also writes `schema_{prefix}.sql` with the prefix already substituted. Charset `utf8mb4`. Engine InnoDB.

`sort` belongs in the first `CREATE`. Kingston had to `ALTER` it later because the first diary write omitted it.
...
Bodies are text. A section of a few thousand characters is right. Do not store a PDF or a screenshot dump in MySQL. Store the path and the SHA-256.
...
**After the write.** Delete one-shot dump files. Keep the local payload mirror and the Commander?s paste file.

---

## 8. Local files (the spare copy)

Keep these next to each other, in the write-root (this kit folder after the Commander copied it):

| File | Holds |
|---|---|
| `_prepared.json` | Name, prefix, write-root, origin, database, user. **No password.** Written by `prepare_agent.py`. |
| `payload_skeleton.py` then `_{name}_self_payload.py` | The same pages and JSON as the tables. **No passwords.** |
| On-screen first-save | Printed by `prepare_agent.py`. **Once.** Not a file. |
| `Revive.md` | Revive paste for later new chats. **No passwords.** |
| `commander.md` | Pointer only. Not a paste. |
| This manual | How to do it again |

The live locker is the database. The folder is the map and the spare text if the site is down. The job?s files (PDFs, plates, inventories) live in the write-root, not inside the locker tables.
...
4. Open the local payload file and confirm it matches the tables.
5. Confirm the forbidden tree was not modified. Confirm you did not upload.
...
The Commander must paste this into a **new** chat. After prepare, `Revive.md` already has the braces filled. Keep the claves out of the table; he attaches them in the chat.
...
Store both texts in `09_restore_prompt` and in `Revive.md`.
...
9. Ask for claves if a live login is required. Do not guess. If a login fails once, stop and report.

Browser tools may be missing. HTTP to phpMyAdmin is enough.
...
These happened on 29?30/08/2026. Do not pay again.
...
| Hard-coded Biblia path in the Commander paste | The kit moves. Playbook path is **this** write-root. |
| ?Rewrite the soul? / ?first locker? as a later self-task | Next wake can read that as a wipe. First locker is only a new man on an empty database. |
...
- Do not rebuild finished work because you woke.
...
- Do not copy KingstonDepp?s pages into a new man.
...
- [ ] Paste pack in `09_restore_prompt` and `Revive.md`
...
Si el 3306 est? cerrado, phpMyAdmin. Escrib? por `/import`. Le? por `/export`. No conf?es en el HTML de `/sql`.
...
La primera guarda de KingstonDepp fue un diario. No alcanz?. La segunda, a las 20:53 ART del 29/08/2026, s?. El locker vivo de Kingston es `neonvintage_kingston`. No lo toques.
...
| Path | What |
|---|---|
| KingstonDepp write-root | `D:\Users\Syn\Nextcloud_sitrep\MorePython\webgames\LHDLT\.A1\docs\Biblia` |
| Live locker | `neonvintage_kingston` @ https://neonvintage.com.ar |
| Retired locker | `siral_kingston` @ https://siral.com.ar |
